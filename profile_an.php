...
<body>
    ...
    // Préparer la requête SQL pour sélectionner l'utilisateur connecté
    $sql = "SELECT * FROM users WHERE id_user='$id_user'";
    $result = $conn->query($sql);

    // Vérifier si la requête a réussi
    if ($result === FALSE) {
        echo "Erreur lors de la récupération de l'utilisateur : " . $conn->error;
    } else {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "Aucun utilisateur trouvé avec l'ID spécifié.";
            exit();
        }
    ...
    <form action="update_profile_an.php" method="POST" class="max-w-md mx-auto bg-gray-100 p-16 rounded-lg m-12">
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="role" id="role" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value="<?php echo $role; ?>" required readonly/>
        ...
        <div class="flex justify-between">
            <a href="announcer.php" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Back</a>
